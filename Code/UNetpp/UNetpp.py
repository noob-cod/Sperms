...
class UNetPP:
    ...
    def get_model(self):
        ...
        while floor != self.depth-1:
            ...
            for inner_ind in range(floor+1):
                ...
                if inner_ind == 0:  # 每个floor的首个单元的输入只有1个，即为上一个floor的首个单元的Maxpooling结果
                    ...
                    x = UpSampling2D(size=[2, 2], interpolation='bilinear')(neurons[floor_ind+1][inner_ind-1])
            ...
            # stage_output = Conv2D(self.out_dim, (1, 1), activation='sigmoid')(neurons[0][floor])
        ...
        #         outputs += stage_output
        #
        # outputs = outputs / (self.depth - 1)

        if self.deep_supervision:
            outputs = []
            for ind in range(1, self.depth):
                outputs.append(
                    Conv2D(self.out_dim, (1, 1), activation='sigmoid')(neurons[0][ind])
                )
        else:
